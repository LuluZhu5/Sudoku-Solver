...
public class Board{
	...
	public void guess() throws Exception
	{
		...
		boardSaves[numberOfGuesses++] = copy;
			...
			for(int y = 0; y < 9; y++) {
				if(board[x][y].getNumber() == 0) {
					...
					if(errorFound() == true) {
						for(int a = 0; a < 9; a++) {
							for(int b = 0; b < 9; b++) {
								board[a][b].setNumber(boardSaves[numberOfGuesses-1].board[a][b].getNumber());
								for(int pot = 1; pot < 10; pot++)
									board[a][b].setPotential(pot, boardSaves[numberOfGuesses-1].board[a][b].canBe(pot));							}
						}
						board[x][y].cantBe(board[x][y].getFirstPotential());
						guess();
					}
					
					else if(errorFound() == false && isSolved() == false) {
						guess();
					}
					
					else if(errorFound() == false && isSolved() == true) {
						return;
					}
				}
			}
